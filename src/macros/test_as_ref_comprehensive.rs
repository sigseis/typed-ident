macro_rules! test_as_ref_comprehensive {
    ($from:ident as $to:ident) => {{
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        let fragment = $from::<Standard, LowLine, Mixed<Ascii>>::new("a")?;
        let _: &$to<Standard, LowLine, Mixed<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        let fragment = $from::<Standard, AsciiFlatLine, Mixed<Ascii>>::new("a")?;
        let _: &$to<Standard, AsciiFlatLine, Mixed<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        let fragment = $from::<Standard, HyphenMinus, Mixed<Ascii>>::new("a")?;
        let _: &$to<Standard, HyphenMinus, Mixed<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        let fragment = $from::<Standard, LowLine, Mixed<Strict>>::new("a")?;
        let _: &$to<Standard, LowLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        let fragment = $from::<Standard, AsciiFlatLine, Mixed<Strict>>::new("a")?;
        let _: &$to<Standard, AsciiFlatLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        let fragment = $from::<Standard, HyphenMinus, Mixed<Strict>>::new("a")?;
        let _: &$to<Standard, HyphenMinus, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Strict>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        let fragment = $from::<Standard, LowLine, Mixed<Unicode>>::new("a")?;
        let _: &$to<Standard, LowLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        let fragment = $from::<Standard, AsciiFlatLine, Mixed<Unicode>>::new("a")?;
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();

        let fragment = $from::<Standard, HyphenMinus, Mixed<Unicode>>::new("a")?;
        let _: &$to<Standard, HyphenMinus, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();
        ...
        let _: &$to<Standard, LowLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Uniform<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Uniform<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Uniform<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Uniform<Strict>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Uniform<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Uniform<Unicode>> = fragment.as_ref();
        ...
        let _: &$to<Standard, HyphenMinus, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Uniform<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Uniform<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Uniform<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Uniform<Strict>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Uniform<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Uniform<Unicode>> = fragment.as_ref();
        ...
        let _: &$to<Standard, LowLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Camel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Camel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Camel<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Camel<Strict>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, Camel<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Camel<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, LowerCamel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, LowerCamel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, LowLine, LowerCamel<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, LowerCamel<Strict>> = fragment.as_ref();
        ...
        let _: &$to<Standard, HyphenMinus, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Mixed<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Camel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Camel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Camel<Strict>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Camel<Strict>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, Camel<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, Camel<Unicode>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, LowerCamel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, AsciiFlatLine, LowerCamel<Ascii>> = fragment.as_ref();
        let _: &$to<Standard, HyphenMinus, LowerCamel<Strict>> = fragment.as_ref();
